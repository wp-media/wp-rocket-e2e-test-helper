<div class="row">
    <div class="col-sm-7">
        <form action="<?php echo esc_url( $this->form_data['form_action'] ) ?>" method="POST" id="<?php echo esc_attr( CONFIG['PLUGIN_ID'] ); ?>_filters_form">
            ...
            <div class="mb-3 w-50">
                <label for="license_creation_date_override" class="form-label">License creation date:</label>
                <select class="form-select" name="license_creation_date_override" id="license_creation_date_override" aria-label="License creation date override">
                    <?php foreach ( $this->form_data['filters']['license_creation_date_override']['form_data'] as $key => $value ) : ?>
                        <option value="<?php echo esc_attr( $key ); ?>" <?php echo $key === $this->form_data['filters']['license_creation_date_override']['state'] ? 'selected="selected"' : '' ?> ><?php echo esc_html( $value ); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
